...
module Section = struct
  type t = { description: string; changes: Change.t list }

  let pp f { description; changes } =
    Fmt.pf f "## %s\n %a" description Fmt.(list ~sep:(unit "\n") Change.pp) changes
end
...
module Release = struct
  type date =
    ...
    | Custom of string (* unreleased *)

  type header = { version: string; date: date option }
  type t =
    | ReleaseChange of header * Change.t list
    | ReleaseSectionChange of header * Section.t list
  ...
  let pp f = function
    | ReleaseChange (header, changes) ->
       Fmt.pf f "# %a\n%a" pp_header header Fmt.(list ~sep:(unit "\n") Change.pp) changes
    | ReleaseSectionChange (header, sections) ->
       Fmt.pf f "# %a\n%a" pp_header header Fmt.(list ~sep:(unit "\n") Section.pp) sections
end
...
module Parser = struct
  ...
  let version_char =
    (alphanum <|> char ' ' <|> char '.' <|> char '~' <|> char '+' <|> char ',' <|> char '"' <|> char '#') <?> "Version character"
  ...
  let version =
    many1_chars (not_followed_by (char '(' <|> char '[' <|> colon) "" *> version_char)
    |>> String.trim
  ...
  let atx_release_header =
    ...
    option colon *> skip_many newline *>
    return {Release.version; date}

  let setext_release_header =
    release_version >>= fun (version, date) ->
    newline *> many1_chars (char '-' <|> char '=') *>
    skip_many1 newline *>
    return {Release.version; date}
  ...
  let ascii_header =
     ...
     *)
    colon *> skip_many1 newline *>
    return {Release.version; date}

  let release_header =
    (atx_release_header <|> attempt setext_release_header <|> ascii_header
    ) <?> "release header"
  ...
  let rec continue_change d prev_lines =
    ...
    followed_by (many1 newline *> release_header) "next line not new release_header" <|>
    followed_by (blank_line *> not_followed_by (skip_count d.cur_change_d blank) "") "next line not new release" <|>
  ...
  (* let section_header = *)
  (*   (string "###" *> line <* colon <* newline) <|> *)
  (*     (string "####" *> line <* newline) <|> *)
  (*     (many1_chars alphanum <* not_followed_by (char '[' <|> char '(') "section_header" <* newline) *)

  (* let section = *)
  (*   section_header >>= fun section_header -> *)
  (*   many change >>= fun changes -> *)
  (*   clear_bullet_state >> *)
  (*   many newline >> *)
  (*     return @@ {Section.description = section_header; changes} *)

  (* let release_with_sections header = *)
  (*   many_until section release_header >>= fun sections -> *)
  (*   return @@ Release.ReleaseSectionChange (header, sections) *)

  let release header =
    many change >>= fun changes ->
    clear_bullet_state >>
    many newline >>
    (* TODO Should this be clearing indentation like it clears bullet_state ? *)
    return @@ Release.ReleaseChange (header, changes)

  let release =
    release_header >>= fun header -> release header

    (* (release header <|> release_with_sections header) *)

  let releases =
    many_until release eof
